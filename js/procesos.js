document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`/.netlify/functions/getData?rol=${user.Rol}&userID=${user.UserID}`)
        ...
        .then(data => {
            ...
            rawDataContainer.textContent = JSON.stringify(tacticosKpis, null, 2);
            
            if (!tacticosKpis || tacticosKpis.length === 0) {
                 kpiContainer.innerHTML = '<p>No se encontraron KPIs tácticos.</p>';
                 return;
            }

            // --- INICIO DE LÓGICA COPIADA DE MAIN.JS ---
            // (Aplicada al array 'tacticosKpis')
            ...
            tacticosKpis.forEach(kpi => {
                ...
                // La clase kpiTypeClass siempre será la de 'táctico' aquí
