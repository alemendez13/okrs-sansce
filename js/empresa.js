...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Guarda de seguridad, solo admins ven esta página
    ...
    headerTitle.textContent = 'Resultados de la Empresa'; // Título de la página
    ...
    fetch(`/.netlify/functions/getData?rol=${user.Rol}&userID=${user.UserID}`)
        ...
        .then(data => {
            ...
            data.forEach(objetivo => {
                // --- INICIO DE LA MODIFICACIÓN (FASE 2) ---
                ...
                objetivo.ResultadosClave.forEach(kr => {
                    // FILTRO: Esta página solo muestra KPIs 'estratégico'
                    const strategicKpis = kr.KPIs.filter(kpi => kpi.kpi_type === 'estratégico');
                    
                    // Si no hay KPIs estratégicos en este KR, no mostramos el KR
